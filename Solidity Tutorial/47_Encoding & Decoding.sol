...
contract Encoding {
    ...
    // Let's compile this contract in hardhat or remix, and you'll see the the "bytecode" output - that's that
    // will be sent when creating a contract.
    ...
    // Which is why so many blockchains are able to do this - you just get them to be able to understand
    // the EVM and presto! Solidity smart contracts work on those blockchains.

    // Now, just the binary can be hard to read, so why not press the `assembly` button? You'll get the
    // binary translated into the opcodes and inputs for us!
    // It's important to know to understand how to build more complex apps.
    ...
    function encodeStringPacked() public pure returns(bytes memory) {
        bytes memory someString = abi.encodePacked("some strign");
        return someString;
    }
    ...
    function multiDecode() public pure returns(string memory, string memory) {
        (string memory someString, string memory someOtherString) =
